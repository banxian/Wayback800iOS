...
#include <QtGui/private/qapplication_p.h> // include path
...
char ansinull[2] = "";
char* strnull = &ansinull[0];

// code snippets take from qapplication_qpa.cpp
// need QtGui n mkspecs in include path
static void init_platform_lite(const QString &name, const QString &platformPluginPath)
{
...
static void cleanup_platform_lite()
{
...
TMainViewBridge::TMainViewBridge(void *view)
    : fParent(view)
    , argc(0)
    , argv(&strnull)
    , fakeApp(argc, argv)
...
{
    //fakeApp.exec();
    //fakeApp.processEvents(); // call in viewcontroller
    init_platform_lite(QLatin1String("uikitlite"), ""); // this make CoreText avalible by initialize platform_integration.
    //init_plugins(pluginList); // jpegmod n pngmod
    
    QColormap::initialize();
    QFont::initialize();
    
    CGSize stagesize = ((MainViewController*)(__bridge id)fParent).stageSize;
    
    fStageProxy = QImage(stagesize.width, stagesize.height, QImage::Format_RGB32);
    fStageProxy.fill(0);
    //LoadAppSettings();
...
TMainViewBridge::~TMainViewBridge()
{
    //SaveAppSettings();
...
void TMainViewBridge::onBenchmarkClicked()
{
    
}

void TMainViewBridge::onEmulationStartClicked()
{
...
void TMainViewBridge::onEmulationRestartClicked()
{
    theNekoDriver->StopEmulation();
    theNekoDriver->StartEmulation();
}
...
void TMainViewBridge::onLCDBufferChanged( QByteArray* buffer )
{
    // TODO: Anti alias
    ...
    //ui->lcdView->repaint();
    painter.begin(&fStageProxy);
    painter.drawImage(0, 0, via);
    painter.end();
...
void TMainViewBridge::DrawShadowOrPixel( QByteArray* buffer, QPainter &painter, bool semishadow )
{
    if (semishadow) {
...
void TMainViewBridge::keyPressEvent( QKeyEvent * ev )
{
...
void TMainViewBridge::keyReleaseEvent( QKeyEvent * ev )
{
    fPressedKeys -= (Qt::Key)ev->key();
...
bool TMainViewBridge::eventFilter( QObject*, QEvent* ev )
{
...
void TMainViewBridge::repaintKeypad()
{
    QImage image(fKeypadRect.size(), QImage::Format_RGB32);
    image.fill(Qt::white);
    for (int y = 0; y < 8; y++) {
        for (int x = 0; x < 8; x++) {
            TKeyItem* item = fKeyItems[y][x];
    ...
    // TODO: copy to stageproxy
    //ui->keypadView->setPixmap(QPixmap::fromImage(image));
    QPainter painter(&fStageProxy);
    painter.drawImage(fKeypadRect.left(), fKeypadRect.top(), image);
    painter.end();
...
void TMainViewBridge::onKeypadSizeChanged(int w, int h)
{
    // reset keypad size
...
void TMainViewBridge::onMouseDown( int x1, int y1 )
{
    bool hitted = false;
    x1 -= fKeypadRect.left();
    y1 -= fKeypadRect.top();
...
void TMainViewBridge::onMouseUp( int x1, int y1 )
{
    bool hitted = false;
    x1 -= fKeypadRect.left();
    y1 -= fKeypadRect.top();
...
void TMainViewBridge::onMouseDoubleClick( int x1, int y1 )
{
    
}

void TMainViewBridge::onMouseMove( int x1, int y1 )
{
    
}

void TMainViewBridge::initLcdStripe()
{
